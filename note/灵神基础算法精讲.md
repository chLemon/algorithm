...
[基础算法精讲·题目汇总](https://github.com/EndlessCheng/codeforces-go/blob/master/leetcode/README.md)
...
如果需要上一层的信息，不要在这一层处理；在上一层处理子节点即可
...
- 站在**输入**的角度：每个元素可以在子集中（选），也可以不在子集中（不选）。**叶子是答案。**

![子集型回溯-选不选问题树](images/子集型回溯-选不选问题树.png)
...
递归循环的时间复杂度是：$2^n$，因为每次有2个选择，选n次

##### 6.1.1.2. 思路二：选哪个

- 站在**输出**的角度：枚举第一个数选谁，枚举第二个数是谁。**每个节点都是答案。**

![子集型回溯-选哪个问题树](images/子集型回溯-选哪个问题树.png)

注意：`[1, 2]` 和 `[2, 1]` 是重复的子集，为了避免重复，下一个数应该大于当前选择的数。

这里不考虑顺序，我们就规定一个顺序，以一个方向去选择。

1. 当前操作：枚举一个下标 j >= i 的数字，进行选择
2. 子问题：从下标 >= i 的数字中构造子集
...
写的时候注意，dfs(i) 的含义始终都是问题本身，i 的变化是问题规模的变化。
...
![组合型回溯选哪个问题树](images/组合型回溯-选哪个问题树.png)

组合型回溯可以有2处剪枝：

一是，由于组合长度固定，所以在长度达到后就可以返回了。（左图，选取2个元素的组合时，下方的 `[3,2,1]` 无需遍历）

二是，如何剩余元素不足填充答案，也可以提前返回了。（右图，选取3个元素的组合时，`[1] [2] [2,1]` 都无需遍历）

剪枝一在加入答案后返回即可。

剪枝二，设 path 长度为 m，组合要求 k 个数，那么还需要 d = k - m 个数，从大到小遍历，待选数为 `[1, i]`，`i < d` 时即可返回。

时间复杂度：从问题树上看，时间复杂度可以估算为 $树高 \times 答案叶子数$，即 $kC_n^k$，以右侧树为例，只有 `[3] [3,2] [3,2,1]` 这条路径才会走到。
...
相比组合，排列的元素顺序是有区别的。`[1, 2]` 和 `[2, 1]` 是不一样的答案。

![全排列问题树](images/排列型回溯-全排列问题树.png)

数组 path 记录答案，集合 s 记录剩余未选的数字（或者 bool 数组 on_path 来标记已选情况）

1. 当前操作：从 s 中枚举 `path[i]` 要填入的数字 x
2. 子问题：构造排列 >= i 的部分，i 是答案/问题规模，剩余未选数字集合为 s
3. 下一个子问题：构造排列 >= i + 1 的部分，i 是答案/问题规模，剩余未选数字集合为 `s - {x}`

$$
dfs(i)
\begin{array}{l}
    \nearrow dfs(i+1, s - {x_1}) \\
    \rightarrow dfs(i+2, s - {x_2}) \\
    \searrow \dots
\end{array}
$$
...
之前在组合型计算复杂度时，我们用 $树高 \times 答案叶子数$ 来估算，则此处问题树本身遍历的复杂度可估算为：$n \times A_n^n = n \times n!$。

如果是使用 set （哈希表）来记录哪些元素使用了的写法：
...
def premute(self, nums: List[int]) -> List[List[int]]:
    ...
    def dfs(i, s):
        ...
        for x in s:
            path[i] = x
            dfs(i + 1, s - {x}) # 这里拷贝了 set
    dfs(0, set(nums))
    return ans
```

这次 set 的拷贝也可以放在下一次递归中，那么就和 `for x in s:` 的 for 循环合并起来了。

那么总复杂度估算为：$n^2 A_n^n$

**但是这样估计会重复统计很多个节点**，例如根节点，会在每个路径上都统计一次。这里是可以计算出整个树的节点个数的。

从下往上看这个问题树，最底层的个数是 $A_n^n$，上一层是 $A_n^{n-1}$，所以整个树的节点个数为：
...
$$
A_n^n + A_n^{n-1} + ... + A_n^1 = n! \times (\frac{1}{0!} + \frac{1}{1!} + ... + \frac{1}{n!})
$$
...
$$
\begin{aligned}
节点个数 &= A_n^n + A_n^{n-1} + ... + A_n^1 \\
&= n! \times (\frac{1}{0!} + \frac{1}{1!} + ... + \frac{1}{n!}) \\
&= n! \times (e - (\frac{1}{(n + 1)!} + \frac{1}{(n+2)!} + ... )) \\
&= e \times n! - (\frac{n!}{(n+1)!} + \frac{n!}{(n+2)!} + ...)
\end{aligned}
$$

后面的这部分，在 n > 1 时，可以证明是小于 1 的：

$$
\begin{aligned}
(\frac{n!}{(n+1)!} + \frac{n!}{(n+2)!} + ...) &= \frac{1}{n + 1} + \frac{1}{(n+1)(n+2)} + ... \\
&< \frac{1}{n+1} + \frac{1}{(n+1)^2} + \frac{1}{(n+1)^3} + ... （无穷等比级数）\\
&= \frac{1}{n}
\end{aligned}
$$
...
$$
节点个数 = e \times n! - （某个小于1的数）= \lfloor e \times n! \rfloor
$$

**这里还可以用一个放缩法简单估算一下 big O 记号下的复杂度。**

最后一行和倒数第二行都有 $n!$ 个节点，由于上一层的分叉了2个，所以再上一行是 $\frac{n!}{2}$ 个，再往上分叉变多，所以再上一行的节点个数必定小于 $\frac{n!}{2 * 2}$，那么这里就是个 $\frac{1}{2}$ 的等比数列，整个树的节点树一定小于 $3 n!$ 个。

以上结论可以推广为：只要是分叉大于等于 2 的树，节点个数在大 O 记号下，**节点个数 = 叶子个数**。
...
之前在子集型回溯，我们用 选和不选/选哪个 这两种思路来解决，其实对于很多动态规划问题，也可以用这两种思路来思考。
...
以题目 [打家劫舍](https://leetcode.cn/problems/house-robber/) 为例，小偷只能偷不相邻的房子。

先看作一道回溯题，那么要把一个大问题变成规模更小的子问题，从第一个房子或者最后一个房子开始思考，比较容易，因为他们受到的约束最少。

假设有4栋房子，以最后一个房子开始思考。选，问题规模变为 n - 2 个房子，不选，问题规模变为 n - 1 个房子。

![打家劫舍回溯思路问题树](images/DP-打家劫舍回溯思路问题树.png)

回溯三问：

1. 当前操作：枚举**第** i 个房子选/不选
2. 子问题：从**前** i 个房子中得到的最大金额和
3. 下一个子问题：
   分类讨论：
...
此处没有将金额作为入参，而是作为返回值，是为了后面记忆化作准备。
...
这样的复杂度是 O(2^n)，注意到，问题树里，有些问题计算了多次，例如上图中 2 个房子的结果，就计算了 2 次。可以用一个数组 `cache / memo` 记一下结果。优化后的搜索树如下，只有 O(n) 个节点：

![alt text](images/DP-记忆化搜索搜索树.png)
...
时间复杂度为 $状态个数 \times 单个状态计算需要的时间$。空间复杂度 O(n)。

注意到，这里递归过程里，实际上是最先算出了 dfs(0)，再计算 dfs(1) -> dfs(2) -> ...
...
但是这样下标 i - 1, i - 2 会出现负数，需要对 i = 0, i = 1 的情况特殊处理。为了避免负数下标，可以把 i 改成从 2 开始（在 f 数组前加入 2 个 0），也可以把 i 再加 2，得到下式：

$$
f[i + 2] = max(f[i + 1], f[i] + nums[i])
$$

注意此时 + 2 后 i 的含义并没有改变，依旧代表了问题规模（房子数），这里的 + 2 改变的只是 `f[i]` 的含义：`f[i + 2]` 代表问题规模 i 时的答案，也相当于在 f 数组前加入了 2 个0。
...
此时可以注意到，`f(i)` 的计算，只依赖了上一个结果和上上一个结果，则可以只用 2 个变量来记录。
...
0-1 背包和完全背包是非常重要的 DP 模型，是 选或不选 这个思路的代表。

### 8.1. 0-1 背包

定义：有 n 个物品，第 i 个物品的体积为 `w[i]`，价值为 `v[i]` 的每个物品至多选一个，求体积和不超过 `capacity` 时的最大价值和
...
为了避免负数下标，改变 `f[i][c]` 的语义，给 i 加 1，`f[i + 1][c]` 代表从前 i 个物品中得到的最大价值和
...
def zero_one_knapsack(capacity: int, w: List[int], v: List[int]) -> int:
    ...
    f = [[0] * capacity for _ in range(n + 1)]
    for i in range(n):
...
注意到，`f[i + 1][...]` 的值仅需要 `f[i][...]` 的信息便可以计算得出，且 `f[i][...]` 的信息仅用于计算 `f[i + 1][...]` 的值。所以可以只用 2 行数组。
...
还可以只用一个数组，注意到 `f[i + 1][c]` 是用 `f[i][c]` 和 `f[i][c - x]` 计算得出，即每个值只用它上一个数组，位置左侧的信息计算得出（需要保留，不能覆盖），不会用到右侧的信息，那么就可以只用一个数组从右往左计算，进行状态转移。
...
def zero_one_knapsack(capacity: int, w: List[int], v: List[int]) -> int:
    ...
    for i, x in enumerate(w):
        for c in range(capacity, -1, -1):
            # 此处 f[c] = f[c] 多余，可以写为 c >= x；c >= x 又可以写入 range
            if c < x:
                f[c] = f[c]
            else:
...
有 n **种**物品，第 i 种物品的体积为 `w[i]`，价值为 `v[i]`，每种物品无限次重复选，求体积和不超过 capacity 时的最大价值和

回溯三问：

1. 当前操作？枚举第 i 种物品选一个或不选：不选，剩余容量不变；选，剩余容量减少 `w[i]`
2. 子问题？在剩余容量为 c 时，从前 i 个物品中得到的最大价值和
3. 下一个子问题？分类讨论：
    1. 不选：在剩余容量为 c 时，从前 i - 1 种物品中得到的最大价值和
    2. 选：在剩余容量为 `c - w[i]` 时，从前 i - 1 种物品中得到的最大价值和

$$
dfs(i, c) = max(dfs(i-1, c), dfs(i, c-w[i]) + v[i])
$$

这里和0-1背包的区别就在于，选了一个后，这种物品还可以选。递归到 i
...
此处的递推翻译、空间优化和 0-1背包 是类似的。

$$
f[i + 1][c] = max(f[i][c], f[i + 1][c - w[i]] + v[i])
$$

注意到，这里 `f[i + 1][c]` 的计算，是从上一个数组当前位置，和当前数组左侧的信息算出，需要保留，不能覆盖。所以此处应该从左往右计算。
...
如果是从 区域 1、4 转移过来的话，就是倒序；从 区域 2、3 转移过来的话，就是正序。
...
定义 `f[i]` 为体积恰好为 i 时的最大/最小价值和，初始化 f 为 `[0] + [inf/-inf] * capacity`

`f[0] = 0` 的含义是：体积为 0 时，最大/最小价值和为 0。
...
定义 `f[i]` 为体积恰好为 i 时的方案数，初始化 f 为 `[1] + [0] * capacity`

`f[0] = 1` 的含义是：体积为 0 时，方案数为 1（不选任何物品就是一种方案）。对应回溯里，刚好剩余容量为 0 时，找到了一种方案，返回 1；当剩余容量小于 0 时，没有方案，返回 0。
...
默认情况下，子数组和子串是连续的，子序列不一定是连续的。

本质上还是选和不选，考虑最后一对字母，x和y。可以 不选x不选y，选x不选y，不选x选y，选x选y。每种情况都对应一个子问题。

回溯三问：

1. 当前操作？枚举最后一对字母 `s[i]` 和 `t[j]` 的选/不选情况
2. 子问题？在字符串 s 的前 i 个字母和字符串 t 的前 j 个字母中寻找 LCS 的长度
3. 下一个子问题？分类讨论：
    1. 都不选 `s[i]` 和 `t[j]`：在字符串 s 的前 i - 1 个字母和字符串 t 的前 j - 1 个字母中寻找 LCS 的长度
    2. 选 `s[i]` 不选 `t[j]`：在字符串 s 的前 i 个字母和字符串 t 的前 j - 1 个字母中寻找 LCS 的长度
    3. 不选 `s[i]` 选 `t[j]`：在字符串 s 的前 i - 1 个字母和字符串 t 的前 j 个字母中寻找 LCS 的长度
    4. 选 `s[i]` 和 `t[j]`：如果 `s[i] == t[j]`，则在字符串 s 的前 i - 1 个字母和字符串 t 的前 j - 1 个字母中寻找 LCS 的长度，并加上 1；否则，这种情况不合法，不选
...
dfs(i, j) =
\begin{cases}
\max(dfs(i-1, j), dfs(i, j-1), dfs(i-1, j-1) + 1) & \text{if } s[i] = t[j] \\
\max(dfs(i-1, j), dfs(i, j-1), dfs(i-1, j-1)) & \text{if } s[i] \neq t[j]
\end{cases}
$$

这里还有2个问题：

1. $s[i] = t[j]$ 时，需要考虑只选其中一个的情况吗？不需要。$dfs(i-1, j) <= dfs(i-1, j-1) + 1，dfs(i, j-1) <= dfs(i-1, j-1) + 1$，所以只选其中一个的情况，不会比都选更优。详细证明见视频
2. $s[i] \neq t[j]$ 时，需要考虑都不选的情况吗？不需要，因为只不选一个的情况下，接下来就会递归到都不选的情况。
...
![DP-状态机DP](images/DP-状态机DP.png)

`dfs(i, 0)` 定义为第 i 天结束时（第 i + 1 天开始时），未持有股票的最大利润

`dfs(i, 1)` 定义为第 i 天结束时（第 i + 1 天开始时），持有股票的最大利润
